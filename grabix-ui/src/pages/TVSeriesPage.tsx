...
import { getTvSources, type StreamSource } from "../lib/streamProviders";
...
export default function TVSeriesPage() {
  ...
  const fetchTMDB = async (t: Tab, p = 1) => {
    setLoading(true);
    try {
      const eps: Record<Tab, string> = {
        trending: `/trending/tv/week?page=${p}`,
        popular: `/tv/popular?page=${p}`,
        toprated: `/tv/top_rated?page=${p}`,
        onair: `/tv/on_the_air?page=${p}`,
      };
      const d = await tf(eps[t]);
      setShows(p === 1 ? (d.results ?? []) : prev => [...prev, ...(d.results ?? [])]);
    } catch {
      setShows([]);
    } finally {
      setLoading(false);
  ...
  const searchShows = async (q: string, p = 1) => {
    setLoading(true);
    try {
      const d = await tf(`/search/tv?query=${encodeURIComponent(q)}&page=${p}`);
      setShows(p === 1 ? (d.results ?? []) : prev => [...prev, ...(d.results ?? [])]);
    } catch {
      setShows([]);
    } finally {
...
function SeriesActions({ onPlay, onDownload, favId, favItem }: { onPlay: () => void; onDownload: () => void; favId: string; favItem: { id: string; title: string; poster: string; type: "series"; tmdbId: number } }) {
...
function SeriesDetail({ show, tf, onClose, onPlay }: { show: Show; tf: (ep: string) => Promise<any>; onClose: () => void; onPlay: (player: { title: string; subtitle?: string; poster?: string; sources: StreamSource[] }) => void }) {
  ...
  useEffect(() => {
    tf(`/tv/${show.id}`).then(setFull).catch(() => {});
  }, [show.id, tf]);
      ...
      <div style={{ background: "var(--bg-surface)", borderRadius: 16, width: "100%", maxWidth: 700, maxHeight: "90vh", overflow: "hidden", display: "flex", flexDirection: "column", boxShadow: "var(--shadow-lg)", border: "1px solid var(--border)" }} onClick={e => e.stopPropagation()}>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "0 20px 24px" }}>
          <div style={{ display: "flex", gap: 16, marginTop: data.backdrop_path ? -50 : 20 }}>
            ...
            <div style={{ flex: 1, paddingTop: data.backdrop_path ? 55 : 0, minWidth: 0 }}>
              ...
              <div style={{ display: "flex", gap: 5, flexWrap: "wrap" }}>
                {(data.genres ?? []).slice(0, 4).map((genre: { id: number; name: string }) => <span key={genre.id} style={{ background: "var(--bg-active)", color: "var(--text-accent)", padding: "2px 7px", borderRadius: 10, fontSize: 11, fontWeight: 500 }}>{genre.name}</span>)}
              </div>
            </div>
          ...
          <SeriesActions
            onPlay={() => onPlay({
              title: data.name,
              subtitle: "TV series playback powered by your configured stream providers",
              poster: data.poster_path ? `${IMG_BASE}${data.poster_path}` : undefined,
              sources: getTvSources(show.id),
            })}
            onDownload={() => window.open(`https://vidsrc.to/embed/tv/${show.id}`, "_blank")}
            favId={`series-${show.id}`}
            favItem={{ id: `series-${show.id}`, title: data.name, poster: data.poster_path ? `${IMG_BASE}${data.poster_path}` : "", type: "series", tmdbId: show.id }}
          />
