...
async function findTmdbId(title: string): Promise<number | null> {
  try {
    const response = await fetch(`${TMDB}/search/tv?query=${encodeURIComponent(title)}`, {
...
export default function AnimePage() {
  ...
  const fetchList = async (nextTab: Tab, nextPage = 1) => {
    setLoading(true);
    try {
      const urls: Record<Tab, string> = {
        trending: `${JIKAN}/top/anime?filter=airing&page=${nextPage}&limit=20`,
        toprated: `${JIKAN}/top/anime?page=${nextPage}&limit=20`,
        seasonal: `${JIKAN}/seasons/now?page=${nextPage}&limit=20`,
      };
      const response = await fetch(urls[nextTab]);
      const data = await response.json();
      const nextItems = (data.data ?? []) as Anime[];
  ...
  const fetchSearch = async (nextQuery: string, nextPage = 1) => {
    setLoading(true);
    try {
      const response = await fetch(`${JIKAN}/anime?q=${encodeURIComponent(nextQuery)}&page=${nextPage}&limit=20&sfw=true`);
      const data = await response.json();
      const nextItems = (data.data ?? []) as Anime[];
      setItems((prev) => (nextPage === 1 ? nextItems : [...prev, ...nextItems]));
    } catch {
  ...
  const loadMore = () => {
    ...
    setPage(nextPage);
    if (query) fetchSearch(query, nextPage);
    else fetchList(tab, nextPage);
  };
        ...
        {loading && items.length === 0 ? <LoadingGrid /> : items.length === 0 ? (
          ...
          <>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(150px, 1fr))", gap: 14 }}>
              {items.map((anime) => <AnimeCard key={anime.mal_id} anime={anime} onClick={() => setDetail(anime)} />)}
            </div>
      ...
      <div style={{ background: "var(--bg-surface)", borderRadius: 16, width: "100%", maxWidth: 680, maxHeight: "90vh", overflow: "hidden", display: "flex", flexDirection: "column", boxShadow: "var(--shadow-lg)", border: "1px solid var(--border)" }} onClick={(e) => e.stopPropagation()}>
        <div style={{ display: "flex", gap: 16, padding: "20px 20px 0" }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ display: "flex", gap: 5, flexWrap: "wrap" }}>
              {anime.genres?.slice(0, 4).map((genre) => <span key={genre.name} style={{ background: "var(--bg-active)", color: "var(--text-accent)", padding: "2px 7px", borderRadius: 10, fontSize: 11, fontWeight: 500 }}>{genre.name}</span>)}
            </div>
          </div>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "14px 20px 24px" }}>
          ...
          {finding ? (
            ...
            <div style={{ fontSize: 12, color: "var(--text-success)", marginBottom: 14 }}>Stream available via configured providers</div>
